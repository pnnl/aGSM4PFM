...
n_level = 6; % control the refinement level. !!! For better accuracy, control n_level such that the interface occupies at least 4~5 layers of grid.
c_thr = 0.9; % the threshold value defining interface area. -1 <= c <= 1. 
t_total = 50; % total physical time in simulation
...
dt = (ds/ff)^2/10/gamma; % time step for the explicit scheme
...
for i = 1:steps
    ...
    c=c+dt*mu*D; % update the concentration
    ...
    if mod(i,steps/50) == 0
        cq = griddata(x,y,c-2,xq,yq);
        surf(xq,yq,cq,'EdgeColor','None','facecolor','interp') % Note that the c is shifted to show the mesh.
        hold on
        view(2)
        xlabel('x');
        ylabel('y');
